...
impl FromStr for Query {
    ...
    fn from_str(query: &str) -> Result<Self, Self::Err> {
        parser::parse(query).map_err(|e| anyhow!("{}", e))
    }
}
...
mod parser {
    use crate::{Create, Field, Insert, Query, Select, Selector, Type, Value, Var};
    ...
    fn insert(input: Input) -> Parsed<Insert> {
        ...
        let (input, values) =
            preceded(ikey("values"), delimited(key("("), values, key(")")))(input)?;
...
#[cfg(test)]
mod tests {
    use crate::{Create, Field, Insert, Type, Value};

    use super::{Query, Select, Selector};
